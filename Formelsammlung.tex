...
\begin{multicols*}{4}
	...
	\ccontent{\[\tilde{x}=\begin{cases}x_{\frac{n+1}{2}}, ungerade \\\frac{1}{2}(x_{\frac{n}{2}}+x_{\frac{n}{2}+1}), gerade \end{cases}\]}
	Der Median $\tilde{x}$ minimiert die Funktion
	\ccontent{$\sum \limits_{i=1}^n \vert x_i- \bar{x} \vert$}
	...
	\ccontent{ $Q(a,b,c,...) = \sum \limits_{i=1}^n (y_i - f_{a,b,c,...}(x_i))^2$}
	...
	\ccontent{ $\frac{\delta}{\delta a} Q(a,b) = 0 $}
	\ccontent{ $\frac{\delta}{\delta b} Q(a,b) = 0 $}
	...
	\ccontent{ $P(A) = \sum \limits_{i}^n (P(A \vert B_i) \cdot P(B_i)$}
	...
	\ccontent{ $P(\bar{A \cup B}) = P(\bar{A} \cap \bar{B})$}
	\ccontent{ $P(\bar{A \cap B}) = P(\bar{A} \cup \bar{B})$}
	...
	\ccontent{ Die Binomialverteilung kann mit der Normalverteilung approximiert werden, \textbf{bedingung ist}}
	\ccontent{ $X \sim B(n, p) \stackrel{a}{=} N(n \cdot p, n \cdot p \cdot (1-p))$}
	\ccontent{ falls gilt}
	...
	\subheading{Hypergeometrische Vert. apporximieren}
	...
	\ccontent{ $P(X = n) = (1-p)^{n-1} \cdot p^n$}
	...
	\ccontent{ \[f(t) = \begin{cases} \lambda \cdot e^{- \lambda \cdot t}, t \geq 0 \\ 0, t < 0 \end{cases}\]}
	\ccontent{ und daraus eribt sich die Verteilungsfunktion}
	\ccontent{ $ F(x) = P(T \leq x) = $}
	\ccontent{ \[= \int_{-\infty}^{x} f(t)dt = \begin{cases} 1 - e^{- \lambda \cdot x}, x \geq 0 \\ 0, x < 0 \end{cases}\]}
	...
	\ccontent{$a^0 = 1$}
	\ccontent{$a^1 = a$}
	\ccontent{$a^m \cdot a^n = a^{m+n}$}
	\ccontent{$(a^n)^m = a^{n \cdot m}$}
	\ccontent{$a^n \cdot b^n = (a \cdot b)^n$}
	\ccontent{$a^{-n} = \frac{1}{n}$}
	\ccontent{$\frac{a^n}{a^m} = a^{n-m}$}
	\ccontent{$a^\frac{b}{n} = \sqrt[n]{b}$}
	
	\subsubheading{Logarithmusregeln}
	\ccontent{$x = \log_{a}{y} \Leftrightarrow y = a^x$}
	\ccontent{$\log{1} = 0$}
	\ccontent{$\log{x \cdot y} = \log{x} + \log{y}$}
	\ccontent{$- \log{x} = \log{\frac{1}{x}}$}
	\ccontent{$\log{\frac{x}{y}} = \log{x} - \log{y}$}
	\ccontent{$\log{x^n} = n \cdot \log{x}$}
	\ccontent{$\log_{a}{x} = \frac{\log{x}} {\log{a}}$}
