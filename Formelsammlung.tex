...
\begin{multicols*}{4}
	...
	\ccontent{\[\tilde{x}=\begin{cases}x_{\frac{n+1}{2}}\\\frac{1}{2}(x_{\frac{n}{2}}+x_{\frac{n}{2}+1})\end{cases}\]}
	...
	\ccontent{$cov(x,y) = S_{xy} = \frac{1}{n-1}\sum \limits_{i=1}^1(x_i-\bar{x})\cdot(y_i-\bar{y})$}
	...
	\ccontent{ Über die Ableitungen lassen sich die Parameter finden welche die vorgegebene Funktion ambesten annähern}
	...
	\ccontent{ Binomialverteilung kann mit der Normalverteilung approximiert werden, bedingung ist}
	...
	\ccontent{ \[P(T \leq x) = \int_{-\infty}^{x} f(t)dt = \begin{cases} 1 - e^{- \lambda \cdot x}, x \geq 0 \\ 0, x < 0 \end{cases}\]}
