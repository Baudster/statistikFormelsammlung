...
\begin{multicols*}{4}
	...
	\ccontent{$\overline{x}=\frac{1}{n}\sum \limits_{i=1}^n x_i=\frac{1}{n}(x_1 + \dots + x_n)$}
	...
	\ccontent{Mit ersetzung von $P(B)$ durch den Satz der totalen Wahrscheinlichkeit:}
	\ccontent{ $P(A \vert B) = \frac{P(B \vert A) \cdot P(A)}{P(B \vert A) \cdot P(A) + P(B \vert \overline{A}) \cdot P(\overline{A})}$ }

	\subsubheading{Satz der totalen Wahrscheinlichkeit}
	\ccontent{ $P(A) = \sum \limits_{i=1}^n (P(A \vert B_i) \cdot P(B_i))$}
	\ccontent{$P(A) = P(A \vert B) \cdot P(B) + P(A \vert \overline{B}) \cdot P(\overline{B})$}
	...
	\ccontent{ Schlüsselwörter sind \textbf{Ereignisse pro Zeiteinheit}, zum Beispiel Anrufe innerhalb bestimmter Zeitspanne oder Durchschnitt Ereignisse x pro y (z.B.: Staubkörner pro cm$^2$)}
	\ccontent{ $X \sim Pois(\lambda)$}
	\ccontent{ $P(X = k) = \pi_\lambda(k) = \frac{\lambda^k}{k!} \cdot e^{-\lambda}$}
	\ccontent{ $P(X \leq k) = \sum \limits_{i=1}^k \frac{\lambda^i}{i!} \cdot e^{-\lambda}$ }
	\ccontent{ $P(X \geq k) = 1 - P(X<k)$ }
	\ccontent{ Eingabe Taschenrechner }
	...
	\ccontent{ $P(a \leq X \leq b) = F(b) - F(a)$ }
	...
	\ccontent{ Fall a) bei Summierten Ereignisse, Fall b) für Durchschnitt}
	\ccontent{ a) $\sum X_i \sim N(n \cdot \mu, n \cdot \sigma^2)$}
	\ccontent{ b) $\overline{X}_n = \frac{X_1+ ... + X_n}{n} = \overline{x} \sim N(\mu, \frac{\sigma^2}{n})$}
	...
	\subsubheading{ ZGS Algorithmus }
	\ccontent{ 
		$n$ : Anzahl der Zufallsvariablen (Stichproben) bestimmen, \\
		$\sigma^2$: Varianz bestimmen,\\
		$\mu$: Erwartungswert (Mittelwert) bestimmen \\
		Wonach wird gesucht? Summe oder Durchschnitt
	\begin{enumerate}
		\item \textbf{Summe}: \\
		Erwartungswert der Summe bestimmen mit $E(S) = n \cdot \mu$ alternativ und gleichwertig ist: $E(S) = \sum \limits_{i=1}^n E(X_i)$ \\
		Falls notwendig Varianz aus Erwartungswert berechnen: $Var(X) = E(X^2)-E(X)^2$ \\
		Wahrscheinlichkeit normalverteilt berechnen mit $ N(n \cdot \mu, n \cdot \sigma^2)$
		\item \textbf{Durchschnitt}: \\
		Durchschnittserwartungswert bestimmen entwder gegeben oder $\frac{1}{n} \cdot \sum \limits_{i=1}^n E(X_i)$\\
		Falls notwendig (nicht gegeben) Varianz berechnen. \\
		Wahrscheinlichkeit normalverteilt berechnen mit $N(\mu, \frac{\sigma^2}{n})$
	\end{enumerate}
	}
	\ccontent{\textbf{Hinweis} Normalverteilung berechnen: $P(X \leq k) = \Phi(\frac{k-\mu}{\sigma})$}
	...
	\ccontent{Vorbereitung: Falls $F(x)$ gegeben leite ab um $f(x)$ zu erhalten ($f(x_i)$ muss eine \textbf{Dichtefunktion} sein)}
	...
	Achtung die Standartabweichung $\sigma$ bezieht sich im Folgenden auf die Grundgesammtheit! Es kann also vorkommen, das eine Standartabweichung gegeben ist, dann ist allerdings noch zu Prüfen ob es sich um die der Stichprobe oder der Grundgesammtheit handelt. \\
	Mit $\alpha$ in den Intervallformeln ist immer die Irrtumswahrscheinlichkeit gemeint.	
	\subsubheading{Intervall für \boldmath$E(X)$ einer Normalverteilung}
	\ccontent{Wenn nach Schätzintervall des \textbf{Erwartungswerts} gefragt ist. Prüfen ob Sigma bekannt oder nicht und danach Formel wählen. }
	...
	\ccontent{$\alpha = $ Signifikanzwahrscheinlichkeit (Irrtumswahrscheinlichkeit) oder $\alpha = 1 - $Konfidenzniveau}
	\ccontent{alternativ: $1-\alpha = $ Konfidenzniveau (Vertrauensniveau)}
	\ccontent{Ist $\alpha$ bekannt, berechne das Quantil $z_{1-(\alpha / 2)}$}
	...
	\subsubheading{Intervall für \boldmath$Var(X)$ einer Normalverteilung}
	\ccontent{Wenn nach Schätzintervall der \textbf{Varianz} gefragt ist.}
	\ccontent{$\left[\frac{(n-1) \cdot s^2}{\chi^2_{n-1;1-\frac{\alpha}{2}}}, \frac{(n-1) \cdot s^2}{\chi^2_{n-1;\frac{\alpha}{2}}} \right]$}
	...
	\ccontent{Wähle Fall b) wenn $\overline{X} \geq \mu_0$}
	\ccontent{Wähle Fall c) wenn $\overline{X} \leq \mu_0$}
	...
	\begin{tabularx}{\columnwidth}{l|c|c}
		 ...
		 \cline{1-3}
		 $\overline{X} \geq \mu_0$ & $\mu \leq \mu_0$ & $T > Z_{1-\alpha}$ \\
		 \cline{1-3}
		 $\overline{X} \leq \mu_0$ & $\mu \geq \mu_0$ & $T < -Z_{1-\alpha}$ \\
	\end{tabularx}
	...
	\begin{tabularx}{\columnwidth}{l|c|c|c}
		...
		\cline{1-4}
		$\overline{X} \geq \mu_0$ & $\mu \leq \mu_0$ & $T > t_{1-\alpha}$ &$(n-1)$\\
		\cline{1-4}
		$\overline{X} \leq \mu_0$ & $\mu \geq \mu_0$ & $T < -t_{1-\alpha}$ &$(n-1)$\\
	\end{tabularx}
...
\begin{minipage}[b]{\linewidth}
	...
	\cline{1-1}
	\begin{center}$\sum \limits_{i=1}^n -x_i =  -\sum \limits_{i=1}^n x_i$\end{center} \\
	\cline{1-1}	
	\begin{center}$ln(a \cdot \prod \limits_{i=1}^n x_i ) = ln(a)+ \sum \limits_{i=1}^n x_i$\end{center} \\
	\cline{1-1}	
	\begin{center}$\frac{a}{b}/c = \frac{a}{b} \cdot \frac{1}{c}$\end{center} \\
	\cline{1-1}	
	\end{tabularx}
	\newpage
	\heading{Beispiele}
	\subheading{Zentraler Grenzwertsatz}
	\paragraph{Angabe} Seien $T_1, ..., T_{102}$ unabhängige 
	Zufallsvariablen, die alle exponentialverteilt sind mit Rate 
	$\lambda = 3$. Berechnen Sie Mittelwert und Standartabweichung 
	von $S := T_1 + ... + T_{102}$ und $\overline{T} := \frac{1}{102}S$. 
	Berechnen Sie die Wahrscheinlichkeiten $P(S>40)$ und 
	...
	1) $S = \sum \limits_{i=1}^{102} T_i \sim N(\overbrace{102\cdot \frac{1}{3}}^{\mu_S}|\overbrace{102\cdot \frac{1}{9}}^{\sigma^2_S})$ \\
	...
	Standardabweichung von $\overline{T}$: $\sqrt{\frac{1}{918}}$ \\
	\begin{align*} 
		P(S>40) &= 1-P(S\leq 40) \\ 
