...
\begin{multicols*}{4}
	...
	\ccontent{$f(x_i)$ muss eine \textbf{Dichtefunktion} sein}
	...
	\ccontent{$\left[\overline{x} - t_{1-(\alpha / 2)} \cdot \frac{s}{\sqrt{n}}, \overline{x} + t_{1-(\alpha / 2)} \cdot \frac{s}{\sqrt{n}}\right]$}
	...
	\subsubheading{Gauß-Test ($z$-Test, $u$-Test)}
	\ccontent{Ist ein Test für den Erwartungswert einer \textit{Normalverteilung} bei bekannter Standardabweichung $\sigma$}
	\ccontent{Formuliere die Hyptohesen:}
	\ccontent{a) $H_0:\mu = \mu_0$ gegen $H_1:\mu \neq \mu_0$}
	\ccontent{b) $H_0:\mu \geq \mu_0$ gegen $H_1:\mu < \mu_0$}
	\ccontent{c) $H_0:\mu \leq \mu_0$ gegen $H_1:\mu > \mu_0$}
	...
	\ccontent{$z=\frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}}$}
	\ccontent{Bestimme das entsprechende Quantil der Standartnormalverteilung:}
	\ccontent{a) $z_{1-\frac{\alpha}{2}}$ bzw. b), c) $z_{1-\alpha}$}
	\ccontent{Entscheidungsregel: $H_0$ ist zu verwerfen, falls}
	...
	\ccontent{$t=\frac{\overline{x}-\mu_0}{s/\sqrt{n}}$}
	\ccontent{Bestimme das entsprechende Quantil der $t$-Verteilung:}
	\ccontent{a) $t_{n-1;1-\frac{\alpha}{2}}$ bzw. b), c) $t_{n-1;1-\alpha}$}
	\ccontent{Entscheidungsregel: $H_0$ ist zu verwerfen, falls}
	...
	\ccontent{Wegen ZGS:}
	\ccontent{$Z=\frac{\overline{X} - \mu}{\sigma / \sqrt{n}} = \frac{\overline{X} - \mu}{\sigma} \cdot \sqrt{n} \sim N(0,1)$}
	\ccontent{ist zu  gegebenen Konfidenzniveau $1 - \alpha$ das Konfidenzintervall gleich}
	\ccontent{$\left[ \overline{x} - z_{1 - \frac{\alpha}{2}} \cdot \frac{\sigma}{\sqrt{n}}, \overline{x} + z_{1 - \frac{\alpha}{2}} \cdot \frac{\sigma}{\sqrt{n}}\right]$}
	\ccontent{wobei $z_{1-\frac{\alpha}{2}}$ ein Quantil der Standartnormalverteilung ist.}
	\ccontent{Das Konfidenzintervall überdeckt den gesuchten Erwartungswert $\mu$ mit der Wahrscheinlichkeit $1-\alpha$.}
	\subsubheading{Einseitige Konfidenzintervalle}
	\ccontent{$\left] - \infty, \overline{x} + z_{1 - \alpha} \cdot \frac{\sigma}{\sqrt{n}} \right]$ bzw. $\left[ - \infty, \overline{x} - z_{1 - \alpha} \cdot \frac{\sigma}{\sqrt{n}} \right[$}
